...
class RepeatFirstKDataset(Dataset):
...
def train(model, seq_len, vocab_size=vocab_size, act_loss_coeff=0.01):
    dataset = RepeatFirstKDataset(num_samples=10_000, seq_len=seq_len, vocab_size=vocab_size)
...
def evaluate(model, seq_len, vocab_size=vocab_size, n=10, show_results=True):
    ...
    for _ in range(n):
        ...
        target = x[0]
        if show_results:
            print(f"ponder={format_aux(aux)} | x={x.tolist()} | preds={preds.tolist()} | target={target}")
