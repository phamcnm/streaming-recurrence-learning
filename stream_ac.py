...
def create_env(env_id, corridor_length=9, render=False):
    if env_id == "RepeatFirstBlankLast":
        env = RepeatFirstBlankLastEnv()
    elif env_id == "CopyTask":
        env = CopyTaskEnv()
    else:
        env = gym.make(env_id, render_mode='human', max_episode_steps=10_000) if render else gym.make(env_id, max_episode_steps=10_000)
    return env

def main(exp_id, exp_name, seed, env_id, corridor_length, total_timesteps, total_episodes, log_interval,
    ...
    env = create_env(env_id, corridor_length, render)
    ...
    if total_episodes is None or total_episodes == 0:
        total_episodes = total_timesteps//(corridor_length+1)
    iteration_returns = np.zeros(total_episodes//log_interval)
    critic_accuracy = np.zeros(total_episodes//log_interval)
    iteration_timesteps = (np.arange(total_episodes//log_interval)+1) * log_interval * (corridor_length+1)

    repetitions = 3 * (repeat_mode in ['ds_adaptive', 'ds_fixed']) + 1
    num_deep_supervision_steps = np.zeros((total_episodes, corridor_length+1))
    num_ponder_steps = np.full((total_episodes, corridor_length+1), inner_loops)
    ...
    while episode_count < total_episodes:
        ...
        num_deep_supervision_steps[episode_count, step_count%(corridor_length+1)] = repeat+1
        if aux_data is not None:
            ponder_steps = aux_data if isinstance(aux_data, int) else aux_data['summary'][0]
            num_ponder_steps[episode_count, step_count%(corridor_length+1)] = ponder_steps
        ...
        if terminated or truncated:
            ...
            if episode_count % log_interval == 0: # logging
                ...
                print(f"    Ponder {ponder_steps if corridor_length<10 else ponder_steps[:6] + ['...'] + ponder_steps[-5:]}")
                print(f"    Repeat {repeat_steps if corridor_length<10 else repeat_steps[:6] + ['...'] + repeat_steps[-5:]}\n")
                last_log_time = time.time()
                last_step_count = step_count

                if debug:
                    env_stats = {'gamma': gamma, 'obs_stats': env.env.obs_stats.get_stats(), 'rew_stats': env.env.env.reward_stats.get_stats()} # merge
                    # avg_returns, critic_mse, ims = evaluate_critic(env_id, env_stats, corridor_length, agent, ponder_mode, inner_loops, repeat_mode, fig, axs, ims, showfig=showfig)
    ...
    run_dir = os.path.join(exp_dir, f"{env_id}_{corridor_length}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--corridor_length', type=int, default=9, help='Length of the corridor in the TMaze environment')
    ...
    # rnn args
    parser.add_argument('--rnn_type', type=str, default='lru')
    ...
    main(args.exp_id, args.exp_name, args.seed, args.env_id, args.corridor_length, args.total_timesteps, args.total_episodes, args.log_interval,
