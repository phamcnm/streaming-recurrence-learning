...
class DropinRNN(nn.Module):
    def __init__(self, in_features, out_features, state_features=None, bias=True, nonlinearity="tanh", **kwargs):
        super().__init__()
    ...
    def forward(self, x, hidden=None, done=None, **kwargs):
        ...
        if hidden is None:
            hidden = torch.zeros(1, batch_size, self.rnn.hidden_size, device=device, dtype=dtype)
        elif hidden.dim() == 2:
        ...
        if done is None:
            ...
            for t in range(seq_len):
                y_t, hidden = self.rnn(x[t:t + 1], hidden)
                outs.append(y_t)
